...
async function loadAttendance() {
    ...
    data.forEach(record => {
      table.innerHTML += `
        <tr>
          ...
          <td><span class="status ${record.method === "manual" ? "pending" : "active"}">${record.method === "manual" ? "Manual" : "Quiz"}</span></td>
  ...
  // Refresh the manual-mark dropdown to exclude students already marked
  const alreadyMarkedIds = new Set(data.map(r => r.student_id));
  ...
  const select = document.getElementById("manualAttendanceSelect");
  select.innerHTML = `<option value="">Select student...</option>`;

  (allStudents || [])
    .filter(s => !alreadyMarkedIds.has(s.student_id))
    .forEach(s => {
      select.innerHTML += `<option value="${s.student_id}">${s.full_name}</option>`;
    });
...
document.getElementById("markManualBtn").addEventListener("click", async () => {

  const studentId = document.getElementById("manualAttendanceSelect").value;
